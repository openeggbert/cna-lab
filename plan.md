...
6. **Developer tools:** no *second, bespoke* editor suite (no separate road-graph/mission-graph/timeline editor). Mesh Craft is already a real 3D scene editor (viewport, gizmos, CSG, Walk Mode) and is the primary tool for spatial content; non-spatial content (missions, dialogue) is hand-written data files. At most one small in-game debug/mission-state view tool is added, for runtime debugging only.
