...
| ED-404 | Light components with viewport visualisation | 🔄 | **The lighting half is done and landed with ED-402**; the *visualisation* half is what remains. `SceneLighting.hpp` reads `CNA.Light` -- which had been a descriptor since Phase 1 with nothing reading it -- and reduces a scene to what `IEffectLights` can be told, which is XNA's fixed-function rig: an ambient colour and exactly three directional lights. There is no point light and no spot light, so a point or spot becomes a directional light **aimed at whatever is being drawn** and dimmed by distance; that is position-dependent, which is what a point light *is*, and its limit is that one large model is lit as though it sat at its own origin. A spot's cone is not modelled at all. Falloff reaches zero at the range rather than following inverse square, which never does -- physical is the wrong answer here, since it leaves a lamp faintly tinting the far side of the level and the range control doing nothing visible. Where more than three lights apply, the **three brightest at that point** win, not the nearest and not the first three: document order is not something a user arranges. A scene with no enabled light, or none in range, asks for XNA's `EnableDefaultLighting()` -- black models read as a broken renderer, and the commonest scene of all is one a model has just been dropped into. What is left: drawing a light's direction and range in the viewport so it can be aimed by looking rather than by typing |
