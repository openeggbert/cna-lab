...
All of gates M0-M11 are now fully done at prototype/first-pass fidelity. M12 is instrumented but open: `--profile <json> --profile-scenario mixed` records a deterministic walking/driving/district-load workload on real EasyGL. One Release intro/idle capture passes the 720p/30 FPS minimum at 16.876 ms frame p95, while mixed captures fail at 51.628-57.705 ms despite very small CPU subsystem times; two identical final intro runs also flipped back-to-back between 51.381 ms and 16.897 ms, exposing unstable host/display frame pacing. RAM and district load pass, and full VRAM residency is not yet exposed by CNA. See `docs/performance-baseline.md`. Next: control and diagnose GPU/present/v-sync behavior, isolate scenario phases, and complete VRAM measurement before marking `IG-39-013` done.
