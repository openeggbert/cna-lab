...
- `P`: pause / resume
- `M`: press and release to open / close the explored-area map; it shows `POWER`/`TERMINAL` progress, while red `GOAL` marks a locked exit and cyan `GOAL` an active exit; the `I` + `L` + `M` cheat takes priority even when its keys are pressed gradually
- `Escape`: quit
