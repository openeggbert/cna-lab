...
- A skinned test character (hand-authored glTF, since Mesh Craft/MC3 has no rigging/skinning support) plays "Idle"/"Walk" clips, crossfading between them, from live on-foot input via `cna-extended`'s `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT`, replacing the procedural on-foot player box when the asset is available. No dialogue pose or vehicle-entry animation yet.
