...
All of gates M0-M11 are now fully done at prototype/first-pass fidelity. M12 is instrumented but open: `--profile <json>` plus `--profile-scenario intro|idle|walk|drive|mixed` records deterministic EasyGL workloads, `--vsync on|off` controls the requested presentation interval, and the schema-2 report separates CPU Draw submission, asynchronous real GPU Draw-range timing, EndDraw/Present CPU, and scoped 3D workload counts (draw/state calls, vertices, triangles, instances, submitted visible objects). Historic hardware-backed mixed captures fail at 51.628-57.705 ms despite small subsystem times, while identical intro runs varied between 51.381 ms and 16.897 ms; those captures predate GPU timing and need a controlled rerun. Isolated Xvfb/llvmpipe full-mixed validation passes 30 FPS but cannot qualify real hardware or v-sync. RAM and district load pass. VRAM reporting includes a category breakdown for game-owned resources plus deduplicated imported CNJ buffers and effect-bound textures (394,340 B in the current first-district Xvfb check), but full backend residency is still not exposed by CNA. See `docs/performance-baseline.md`. Next: use the Draw/GPU/Present split on controlled named hardware and obtain backend/external VRAM residency before marking `IG-39-013` done.
