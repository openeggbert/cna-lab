...
- A skinned test character (hand-authored glTF, since Mesh Craft/MC3 has no rigging/skinning support) plays "Idle"/"Walk" clips from live on-foot input via `cna-extended`'s `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT`, replacing the procedural on-foot player box when the asset is available. No blending, dialogue pose, or vehicle-entry animation yet.
...
Gate M6's first proving slice is done; blending, dialogue pose, and vehicle entry/exit are not. `cna-extended` (a sibling repo, extended for this milestone with the owner's explicit go-ahead) gained `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT`, a new ECS component/system pair wrapping cna's general-purpose `Model`+`AnimationPlayer` skinned path -- the one cna's own glTF/CNJ import tools actually populate for a skinned asset -- since the existing `SkinnedModelComponentEXT`/`AnimationSystem3DEXT` only wrap the separate Avatar-specific `SkinnedModelEXT` data model. A hand-authored 3-bone test character (`assets/source/gltf/test_character.gltf`; Mesh Craft/MC3 has no rigging/skinning authoring support, so MC3 is deliberately bypassed here, unlike every other asset in this repo) was converted through `cna_tool_gltf_to_cnj` to `assets/generated/models/cnj/test_character.cnj` and plays "Idle"/"Walk" clips selected from live on-foot input (`IronShadowsGame::Update`), replacing the procedural player box in `PrototypeRenderer` when it loads. Verified: `cna-extended`'s own new tests (clip start/switch/hard-cut logic against a real `AnimationPlayer`, plus a real pixel-level render test) and a `--smoke` run of the full game with no crash (`plan/plan_39-vertical-slice-gates.md` gate M6, task `IS-39-007`; `plan/plan_18-characters-skeletons-and-animation.md` for the itemized breakdown). Explicitly not done: clip blending (both animation systems only support a hard cut), a dialogue-pose clip, vehicle entry/exit animation (the character simply is not drawn while driving, matching the box it replaced), and any interactive/visual verification, since this environment has no display.
