...
- [x] **IG-25-003 P0** — Display dialogue through a proper subtitle UI. *(`SubtitleLayout` / `WrapSubtitleText` / `ComputeSubtitleLayout` (`include/`/`src/UI/Subtitle.hpp/.cpp`) plus `IronGangGame::DrawSubtitle()`: a bottom-centred, word-wrapped subtitle on a dimmed panel sized to its own content, with the speaker's name above it in a distinct colour and the continue prompt beneath. Replaces a single unwrapped `DrawString` at the top-left corner, which the first screenshots caught running off the right edge mid-word. The layout is arithmetic over a fixed-width font, so it is unit-tested without a graphics device; an over-long word is hard-split rather than allowed to overflow. Two rendering fixes fell out: the HUD batch now uses `SamplerState::PointClamp` (filtering an 8x8 bitmap atlas bled neighbouring glyph cells into every space at 2x scale), and the advance is **measured** from the font rather than assumed to be the glyph cell -- the cell is 8px but the advance is 9px, and assuming 8 drew a line wider than the panel it was wrapped for. Not done: no per-line timing, no fade in/out, no speaker portraits, and no accessibility size/background options (`IG-28` scope).)*
