...
// content-pipeline SpriteFont available at runtime).
...
int FontDrawText(std::vector<std::uint8_t>& buf, int bufW, int bufH, int x, int y, const std::string& text,
                  std::uint8_t r, std::uint8_t g, std::uint8_t b, std::uint8_t a, int scale) {
    int cx = x;
    const int glyphAdvance = kBitmapFontGlyphSize * scale;
    for (char ch : text) {
        const int idx = static_cast<unsigned char>(ch) - 0x20;
        if (idx < 0 || idx >= 95) { cx += glyphAdvance; continue; }
        const auto* glyph = kFont8x8[idx];
        for (int row = 0; row < kBitmapFontGlyphSize; ++row) {
            for (int col = 0; col < kBitmapFontGlyphSize; ++col) {
                if (glyph[row] & (1u << col)) {
