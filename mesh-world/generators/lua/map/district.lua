-- SPDX-License-Identifier: MIT
-- Copyright (c) 2026 Robert Vokac and contributors
--
-- M304 (MAP18, 2026-07-10) — Lua child-tile generator for level 11 ("—" in
-- map.md's own level table, but with defined content: "district layout",
-- ~11 km per that table). Registered as "lua.map.child.level11.default" so
-- MapPipeline's level-aware Lua lookup only tries this for level-11 tiles.
--
-- Mirrors country.lua's (M146) generic child-refinement technique and its
-- settlement-placement approach (same non-ocean/non-mountain single-cell
-- suitability check, no C++ Settlements::place() call).
--
-- V1 SIMPLIFICATIONS:
-- * **Quadrant split, not a real partition.** A genuine district layout
--   would need Voronoi-style or feature-aware region growth (like
--   Countries::grow() does at the C++ level, M139-M140) over a shared
--   multi-tile view this per-tile script doesn't have -- see country.lua's
--   own header for why no such algorithm is ported here. Instead this tile
--   is split into its 4 axis-aligned quadrants (NW/NE/SW/SE); each
--   quadrant that has a suitable site becomes its own named district,
--   exported as a Border polygon covering exactly that quadrant, mirroring
--   country.lua's own "border == this tile's/quadrant's own rectangle,
--   not a traced political boundary" honesty.
-- * **One town per district**, not real population/footprint detail (that's
--   city.lua's job, 3 levels down at level 12). A quadrant with no suitable
--   site simply has no district at all -- an all-ocean quadrant isn't
--   forced to have one.
--
-- KNOWN LIMITATIONS (same as every other child script):
-- * ctx does not expose WorldConfig to Lua generators yet — hardcodes the
--   same defaults the other scripts do.
-- * MapBuilder::setBiomeField's M108 constraint overwrites this tile's own
--   boundary rows/columns on the 2 sides that touch the parent's boundary;
--   the OTHER 2 sides rely on this script's own bilinear math matching what
--   the sibling computes, same as the others.

local M = {}
M.id       = "lua.map.child.level11.default"
M.version  = "0.1.0"
M.category = "map"

local GRID_SIZE         = 64
local SEA_LEVEL_M       = 0.0
local EQUATOR_TEMP_C    = 30.0
local POLE_TEMP_C       = -20.0
local MOUNTAIN_ELEV_M   = 2500.0
local DETAIL_OFFSET     = 2000.0
local MOISTURE_OFFSET   = 6000.0
local MAX_SITE_ATTEMPTS = 8
-- Tile bounds are half-open [min,max) (MapValidator rejects a point at
-- exactly max_x/max_z) -- same epsilon country.lua's own border corners use.
local BORDER_EPS_M      = 1e-6

local function bilinear(grid, fx, fy)
    fx = math.max(0.0, math.min(grid.w - 1, fx))
    fy = math.max(0.0, math.min(grid.h - 1, fy))
    local x0 = math.floor(fx)
    local y0 = math.floor(fy)
    local x1 = math.min(x0 + 1, grid.w - 1)
    local y1 = math.min(y0 + 1, grid.h - 1)
    local tx = fx - x0
    local ty = fy - y0
    local function at(gx, gy) return grid.data[gy * grid.w + gx + 1] end
    local v00, v10 = at(x0, y0), at(x1, y0)
    local v01, v11 = at(x0, y1), at(x1, y1)
    local a = v00 + (v10 - v00) * tx
    local b = v01 + (v11 - v01) * tx
    return a + (b - a) * ty
end

-- One suitable (non-ocean, non-mountain) random site within the grid-cell
-- rectangle [gx0,gx1) x [gy0,gy1), or nil if none found within
-- MAX_SITE_ATTEMPTS tries.
local function find_site_in_quadrant(ctx, elevation, W, H, gx0, gx1, gy0, gy1,
                                      child_x0, child_y0, child_size_m)
    for _attempt = 1, MAX_SITE_ATTEMPTS do
        local gx = ctx.randomInt(gx0, gx1 - 1)
        local gy = ctx.randomInt(gy0, gy1 - 1)
        local elev_above_sea = elevation[gy * W + gx + 1] - SEA_LEVEL_M
        if elev_above_sea > 0.0 and elev_above_sea < MOUNTAIN_ELEV_M then
            local wx = child_x0 + (gx + 0.5) * child_size_m / W
            local wz = child_y0 + (gy + 0.5) * child_size_m / H
            return wx, wz
        end
    end
    return nil, nil
end

function M.generate(ctx, map)
    local parent = ctx.parent
    if not parent then return end

    local W, H = GRID_SIZE, GRID_SIZE
    local cx = ctx.tile_x % 2
    local cy = ctx.tile_y % 2

    local planet_size_m = ctx.tile_size_m * (2 ^ ctx.level)
    local child_size_m  = ctx.tile_size_m
    local child_x0      = ctx.tile_x * child_size_m
    local child_y0      = ctx.tile_y * child_size_m

    local terrain_scale = math.max(child_size_m / 4.0, 1.0)
    local detail_amp    = math.min(child_size_m / 100.0, 500.0)

    local elevation, temperature, moisture = {}, {}, {}

    for gy = 0, H - 1 do
        for gx = 0, W - 1 do
            local pgx = cx * 32.0 + gx * (32.0 / (W - 1))
            local pgy = cy * 32.0 + gy * (32.0 / (H - 1))
            local base_elev = bilinear(parent.elevation, pgx, pgy)

            local fx_fade = math.sin(math.pi * gx / (W - 1))
            local fy_fade = math.sin(math.pi * gy / (H - 1))
            local fade    = fx_fade * fx_fade * fy_fade * fy_fade

            local wx = child_x0 + (gx + 0.5) * child_size_m / W
            local wy = child_y0 + (gy + 0.5) * child_size_m / H

            local detail_fbm = ctx.noise(wx / terrain_scale + DETAIL_OFFSET, wy / terrain_scale)
            local elev = base_elev + fade * (detail_fbm - 0.5) * detail_amp

            local lat_factor = math.abs(wy / planet_size_m - 0.5) * 2.0
            local base_temp  = EQUATOR_TEMP_C + (POLE_TEMP_C - EQUATOR_TEMP_C) * lat_factor
            local elev_above = math.max(elev - SEA_LEVEL_M, 0.0)
            local temp       = base_temp - 6.5 * elev_above / 1000.0

            local moisture_fbm = ctx.noise(wx / terrain_scale + MOISTURE_OFFSET, wy / terrain_scale)
            local moist = math.max(0.0, math.min(1.0, moisture_fbm))

            local idx = gy * W + gx + 1
            elevation[idx]   = elev
            temperature[idx] = temp
            moisture[idx]    = moist
        end
    end

    map:setBiomeField(W, H, elevation, temperature, moisture)

    -- 4 axis-aligned quadrants: NW, NE, SW, SE (see header note on why not
    -- a real geometric partition).
    local half = W // 2
    local quadrants = {
        {gx0 = 0,    gx1 = half, gy0 = 0,    gy1 = half}, -- NW
        {gx0 = half, gx1 = W,    gy0 = 0,    gy1 = half}, -- NE
        {gx0 = 0,    gx1 = half, gy0 = half, gy1 = H},    -- SW
        {gx0 = half, gx1 = W,    gy0 = half, gy1 = H},    -- SE
    }

    for i, q in ipairs(quadrants) do
        local site_x, site_z = find_site_in_quadrant(
            ctx, elevation, W, H, q.gx0, q.gx1, q.gy0, q.gy1, child_x0, child_y0, child_size_m)
        if site_x ~= nil then
            local district_name = names.city(parent.culture, ctx.variation + i) .. " District"

            local wx0 = child_x0 + q.gx0 * child_size_m / W
            local wx1 = child_x0 + q.gx1 * child_size_m / W
            local wz0 = child_y0 + q.gy0 * child_size_m / H
            local wz1 = child_y0 + q.gy1 * child_size_m / H
            if q.gx1 == W then wx1 = wx1 - BORDER_EPS_M end
            if q.gy1 == H then wz1 = wz1 - BORDER_EPS_M end

            map:addBorder(district_name, {
                {wx0, wz0}, {wx1, wz0}, {wx1, wz1}, {wx0, wz1},
            })
            map:addCity(names.city(parent.culture, ctx.variation + 100 + i), site_x, site_z, "town")
        end
    end

    map:setMetadata(M.id, parent.culture)
end

return M
