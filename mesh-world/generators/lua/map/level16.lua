...
--
-- M300 (MAP18, 2026-07-10) — Lua child-tile generator for level 16
-- ("block cluster" in map.md's own level table, but "—" for defined
-- content, ~344 m, directly below Neighborhood=15). Registered as
-- "lua.map.child.level16.default" so MapPipeline's level-aware Lua lookup
-- only tries this for level-16 tiles.
--
...
-- inventing a fake name here would overstate what this script actually
-- does. Mirrors every other child script's generic bilinear-from-parent
-- elevation + fade-to-zero fBm detail technique.
--
-- The one thing this level MUST do that a bare generic refinement wouldn't:
-- propagate an inherited "city" zone down from its parent, which one level
-- up is neighborhood.lua's own level-15 output. neighborhood.lua's OWN
-- M354 fix already re-applies whatever its own parent chain (ultimately
-- traced back to city.lua, level 12) told it, so this level's job is
-- simply not to drop that inheritance for its own children -- the exact
-- bug class MAP24's M354 fixed for ChildGenerator.cpp's generic C++ path
-- and neighborhood.lua itself; any NEW dedicated script for a level below
-- city.lua bypasses that C++ path entirely and must reimplement the same
-- read itself (see plan.md's MAP18 intro note, and neighborhood.lua's own
-- M354 section for the pattern this copies).
--
...
local M = {}
M.id       = "lua.map.child.level16.default"
M.version  = "0.1.0"
M.category = "map"
