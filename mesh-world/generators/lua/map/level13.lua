...
--
-- M298 (MAP18, 2026-07-10) — Lua child-tile generator for level 13
-- ("—" in map.md's own level table, ~2.75 km, directly below City=12).
-- Registered as "lua.map.child.level13.default" so MapPipeline's level-
-- aware Lua lookup only tries this for level-13 tiles.
--
...
-- propagate an inherited "city" zone down from its parent (which, one
-- level up, is always city.lua's own level-12 output -- the direct urban
-- footprint city.lua's markUrbanCells() wrote). Without this,
-- BiomeClassifier's purely climate-driven classify() (which has no "city"
-- output at all) would silently overwrite it with a natural biome one
-- level down -- the exact bug class MAP24's M354 fixed for
-- ChildGenerator.cpp's generic C++ path and neighborhood.lua; any NEW
-- dedicated script for a level below city.lua bypasses that C++ path
-- entirely and must reimplement the same read itself (see plan.md's MAP18
-- intro note, and neighborhood.lua's own M354 section for the pattern this
-- copies).
--
...
local M = {}
M.id       = "lua.map.child.level13.default"
M.version  = "0.1.0"
M.category = "map"
