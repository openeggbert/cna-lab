...
//
// M309 (MAP18) — structural tests for generators/lua/map/level1.lua
// (level 1), mirroring CountryLuaTests.cpp's style (child-tile path,
// parent required).
...
std::string read_level1_lua() {
    std::ifstream ifs("generators/lua/map/level1.lua");
...
MapTilePayload run_level1(const MapTilePayload& parent, std::uint64_t entropy,
                                  const TileCoord& tile, std::string* error_out = nullptr) {
    LuaSandbox sandbox;
    return sandbox.executeMap(read_level1_lua(), make_ctx(parent, entropy, tile), error_out);
}
...
TEST(Level1LuaTest, FileReadsAndRunsWithoutError) {
    const std::string source = read_level1_lua();
    ASSERT_FALSE(source.empty()) << "generators/lua/map/level1.lua not found or empty";

    const MapTilePayload parent = make_moderate_land_parent(1);
    std::string error;
    const MapTilePayload payload = run_level1(parent, 2, TileCoord{1, 1, 0}, &error);

    EXPECT_TRUE(error.empty()) << error;
    EXPECT_FALSE(payload.elevation.empty());
    EXPECT_EQ(payload.generator, "lua.map.child.level1.default");
    EXPECT_EQ(payload.culture, "nordic");
}

TEST(Level1LuaTest, FieldShapesMatch64x64) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level1(parent, 2, TileCoord{1, 1, 0});
...
TEST(Level1LuaTest, EdgeDescriptorsMatchElevationBoundary) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level1(parent, 2, TileCoord{1, 1, 0});
...
// M309's genuinely new functionality: 2-4 towns chained by trunk roads.
TEST(Level1LuaTest, EventuallyPlacesTownsAndRoadsAcrossSeveralEntropies) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    bool found_town = false, found_road = false;
    for (std::uint64_t entropy = 1; entropy <= 20 && !(found_town && found_road); ++entropy) {
        const MapTilePayload payload = run_level1(parent, entropy, TileCoord{1, 1, 0});
        for (const auto& f : payload.features) {
...
TEST(Level1LuaTest, RoadCountIsExactlyOneLessThanTownCount) {
    const MapTilePayload parent = make_moderate_land_parent(1);
...
TEST(Level1LuaTest, NeverPlacesATownOrRoadOverOcean) {
    const MapTilePayload parent = make_all_ocean_parent(1);
    for (std::uint64_t entropy = 1; entropy <= 10; ++entropy) {
        const MapTilePayload payload = run_level1(parent, entropy, TileCoord{1, 0, 0});
...
TEST(Level1LuaTest, DeterministicForSameEntropy) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    const MapTilePayload a = run_level1(parent, 999, TileCoord{1, 1, 0});
    const MapTilePayload b = run_level1(parent, 999, TileCoord{1, 1, 0});
...
TEST(Level1LuaTest, MissingParentReturnsEmptyPayloadNotCrash) {
    MapGenContext ctx;
    ctx.tile        = TileCoord{1, 0, 0};
    ctx.entropy     = 1;
    ctx.sea_level_m = 0.0;
    ctx.parent      = nullptr;
    ...
    LuaSandbox sandbox;
    std::string error;
    const MapTilePayload payload = sandbox.executeMap(read_level1_lua(), ctx, &error);
