...
//
// M306 (MAP18) — structural tests for generators/lua/map/trunk_network.lua
// (level 6), mirroring CountryLuaTests.cpp's style (child-tile path,
// parent required).
...
std::string read_trunk_network_lua() {
    std::ifstream ifs("generators/lua/map/trunk_network.lua");
...
MapTilePayload run_trunk_network(const MapTilePayload& parent, std::uint64_t entropy,
                                  const TileCoord& tile, std::string* error_out = nullptr) {
    LuaSandbox sandbox;
    return sandbox.executeMap(read_trunk_network_lua(), make_ctx(parent, entropy, tile), error_out);
}
...
TEST(TrunkNetworkLuaTest, FileReadsAndRunsWithoutError) {
    const std::string source = read_trunk_network_lua();
    ASSERT_FALSE(source.empty()) << "generators/lua/map/trunk_network.lua not found or empty";

    const MapTilePayload parent = make_moderate_land_parent(1);
    std::string error;
    const MapTilePayload payload = run_trunk_network(parent, 2, TileCoord{6, 1, 0}, &error);

    EXPECT_TRUE(error.empty()) << error;
    EXPECT_FALSE(payload.elevation.empty());
    EXPECT_EQ(payload.generator, "lua.map.child.level6.default");
    EXPECT_EQ(payload.culture, "nordic");
}

TEST(TrunkNetworkLuaTest, FieldShapesMatch64x64) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_trunk_network(parent, 2, TileCoord{6, 1, 0});
...
TEST(TrunkNetworkLuaTest, EdgeDescriptorsMatchElevationBoundary) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_trunk_network(parent, 2, TileCoord{6, 1, 0});
...
// M306's genuinely new functionality: 2-4 towns chained by trunk roads.
TEST(TrunkNetworkLuaTest, EventuallyPlacesTownsAndRoadsAcrossSeveralEntropies) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    bool found_town = false, found_road = false;
    for (std::uint64_t entropy = 1; entropy <= 20 && !(found_town && found_road); ++entropy) {
        const MapTilePayload payload = run_trunk_network(parent, entropy, TileCoord{6, 1, 0});
        for (const auto& f : payload.features) {
...
TEST(TrunkNetworkLuaTest, RoadCountIsExactlyOneLessThanTownCount) {
    const MapTilePayload parent = make_moderate_land_parent(1);
...
TEST(TrunkNetworkLuaTest, NeverPlacesATownOrRoadOverOcean) {
    const MapTilePayload parent = make_all_ocean_parent(1);
    for (std::uint64_t entropy = 1; entropy <= 10; ++entropy) {
        const MapTilePayload payload = run_trunk_network(parent, entropy, TileCoord{6, 0, 0});
...
TEST(TrunkNetworkLuaTest, DeterministicForSameEntropy) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    const MapTilePayload a = run_trunk_network(parent, 999, TileCoord{6, 1, 0});
    const MapTilePayload b = run_trunk_network(parent, 999, TileCoord{6, 1, 0});
...
TEST(TrunkNetworkLuaTest, MissingParentReturnsEmptyPayloadNotCrash) {
    MapGenContext ctx;
    ctx.tile        = TileCoord{6, 0, 0};
    ctx.entropy     = 1;
    ctx.sea_level_m = 0.0;
    ctx.parent      = nullptr;
    ...
    LuaSandbox sandbox;
    std::string error;
    const MapTilePayload payload = sandbox.executeMap(read_trunk_network_lua(), ctx, &error);
