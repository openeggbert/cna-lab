...
//
// M298 (MAP18) — structural tests for generators/lua/map/level13.lua
...
std::string read_level13_lua() {
    std::ifstream ifs("generators/lua/map/level13.lua");
...
MapTilePayload run_level13(const MapTilePayload& parent, std::uint64_t entropy,
                            const TileCoord& tile, std::string* error_out = nullptr) {
    LuaSandbox sandbox;
    return sandbox.executeMap(read_level13_lua(), make_ctx(parent, entropy, tile), error_out);
}

} // namespace

TEST(Level13LuaTest, FileReadsAndRunsWithoutError) {
    const std::string source = read_level13_lua();
    ASSERT_FALSE(source.empty()) << "generators/lua/map/level13.lua not found or empty";

    const MapTilePayload parent = make_moderate_land_parent(1);
    std::string error;
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 1, 0}, &error);

    EXPECT_TRUE(error.empty()) << error;
    EXPECT_FALSE(payload.elevation.empty());
    EXPECT_EQ(payload.generator, "lua.map.child.level13.default");
    EXPECT_EQ(payload.culture, "nordic");
}

TEST(Level13LuaTest, FieldShapesMatch64x64) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 1, 0});
    EXPECT_EQ(payload.elevation.w, N);
    EXPECT_EQ(payload.biome.w, N);
    EXPECT_EQ(payload.biome.h, N);
}

TEST(Level13LuaTest, EdgeDescriptorsMatchElevationBoundary) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 1, 0});
...
TEST(Level13LuaTest, NeverInventsCityOverANonUrbanParent) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 1, 0});
...
TEST(Level13LuaTest, InheritsCityFromParentBiomeInteriorOfUrbanHalf) {
    const MapTilePayload parent  = make_half_urban_parent(1);
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 0, 0});  // cx=0: samples parent cols [0,32]
...
TEST(Level13LuaTest, DoesNotInheritCityOutsideTheUrbanParentHalf) {
    const MapTilePayload parent  = make_half_urban_parent(1);
    const MapTilePayload payload = run_level13(parent, 2, TileCoord{13, 1, 0});  // cx=1: samples parent cols [32,64]
...
TEST(Level13LuaTest, DeterministicForSameEntropy) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    const MapTilePayload a = run_level13(parent, 999, TileCoord{13, 1, 0});
    const MapTilePayload b = run_level13(parent, 999, TileCoord{13, 1, 0});
    EXPECT_EQ(a.elevation.data, b.elevation.data);
    EXPECT_EQ(a.biome.data, b.biome.data);
}

TEST(Level13LuaTest, MissingParentReturnsEmptyPayloadNotCrash) {
    MapGenContext ctx;
    ctx.tile        = TileCoord{13, 0, 0};
    ctx.entropy     = 1;
    ctx.sea_level_m = 0.0;
    ctx.parent      = nullptr;
    ...
    LuaSandbox sandbox;
    std::string error;
    const MapTilePayload payload = sandbox.executeMap(read_level13_lua(), ctx, &error);
