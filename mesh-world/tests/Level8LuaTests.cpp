...
//
// M305 (MAP18) — structural tests for generators/lua/map/level8.lua
// (level 8), mirroring CountryLuaTests.cpp's style (child-tile path,
// parent required).
...
std::string read_level8_lua() {
    std::ifstream ifs("generators/lua/map/level8.lua");
...
MapTilePayload run_level8(const MapTilePayload& parent, std::uint64_t entropy,
                                  const TileCoord& tile, std::string* error_out = nullptr) {
    LuaSandbox sandbox;
    return sandbox.executeMap(read_level8_lua(), make_ctx(parent, entropy, tile), error_out);
}
...
TEST(Level8LuaTest, FileReadsAndRunsWithoutError) {
    const std::string source = read_level8_lua();
    ASSERT_FALSE(source.empty()) << "generators/lua/map/level8.lua not found or empty";

    const MapTilePayload parent = make_moderate_land_parent(1);
    std::string error;
    const MapTilePayload payload = run_level8(parent, 2, TileCoord{8, 1, 0}, &error);

    EXPECT_TRUE(error.empty()) << error;
    EXPECT_FALSE(payload.elevation.empty());
    EXPECT_EQ(payload.generator, "lua.map.child.level8.default");
    EXPECT_EQ(payload.culture, "nordic");
}

TEST(Level8LuaTest, FieldShapesMatch64x64) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level8(parent, 2, TileCoord{8, 1, 0});
...
TEST(Level8LuaTest, EdgeDescriptorsMatchElevationBoundary) {
    const MapTilePayload parent  = make_moderate_land_parent(1);
    const MapTilePayload payload = run_level8(parent, 2, TileCoord{8, 1, 0});
...
// M305's genuinely new functionality: 2-4 towns chained by trunk roads.
TEST(Level8LuaTest, EventuallyPlacesTownsAndRoadsAcrossSeveralEntropies) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    bool found_town = false, found_road = false;
    for (std::uint64_t entropy = 1; entropy <= 20 && !(found_town && found_road); ++entropy) {
        const MapTilePayload payload = run_level8(parent, entropy, TileCoord{8, 1, 0});
        for (const auto& f : payload.features) {
...
TEST(Level8LuaTest, RoadCountIsExactlyOneLessThanTownCount) {
    const MapTilePayload parent = make_moderate_land_parent(1);
...
TEST(Level8LuaTest, NeverPlacesATownOrRoadOverOcean) {
    const MapTilePayload parent = make_all_ocean_parent(1);
    for (std::uint64_t entropy = 1; entropy <= 10; ++entropy) {
        const MapTilePayload payload = run_level8(parent, entropy, TileCoord{8, 0, 0});
...
TEST(Level8LuaTest, DeterministicForSameEntropy) {
    const MapTilePayload parent = make_moderate_land_parent(1);
    const MapTilePayload a = run_level8(parent, 999, TileCoord{8, 1, 0});
    const MapTilePayload b = run_level8(parent, 999, TileCoord{8, 1, 0});
...
TEST(Level8LuaTest, MissingParentReturnsEmptyPayloadNotCrash) {
    MapGenContext ctx;
    ctx.tile        = TileCoord{8, 0, 0};
    ctx.entropy     = 1;
    ctx.sea_level_m = 0.0;
    ctx.parent      = nullptr;
    ...
    LuaSandbox sandbox;
    std::string error;
    const MapTilePayload payload = sandbox.executeMap(read_level8_lua(), ctx, &error);
