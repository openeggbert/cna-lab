...
- [ ] **IG-17-015 P1** — Implement impact damage and disabled states, including simple wheel damage as part of the same system (no separate wheel-damage-state machine). *(Partial: impact damage and the disabled state exist (`VehicleDamage`, `include/IronGang/Gameplay/VehicleDamage.hpp`). Impacts are detected from the vehicle's own speed history -- a frame losing speed faster than any brake could **is** a collision -- which needs nothing new from the physics layer and cannot miss a contact Jolt resolved internally. The separation is wide on purpose: braking is about 1 g, the threshold is 4 g, a crash is tens. Integrity runs 1 to 0, accumulates across impacts, floors at 0, and scales the engine's pull down to `minimumSpeedFactor` -- a wrecked sedan still steers and rolls, because stranded is a situation and immobile is a trap. Thresholds are tunable in `sedan.vehicle.json`'s `damage` block; integrity is saved (older saves load an intact car) and published to missions as the `vehicle_integrity`/`vehicle_disabled` facts, so a mission can now fail on a wrecked car. Since 2026-08-26 the countryside mission actually fails on `vehicle_disabled`, so the fact has a user rather than only a test. Verified by `TestVehicleDamageDistinguishesCrashesFromBraking` and by a full `--profile-scenario mission` run that records **zero** impacts and still completes. **Missing: wheel damage** -- wheels have no per-wheel state at all, so there is nothing yet to damage separately.)*
