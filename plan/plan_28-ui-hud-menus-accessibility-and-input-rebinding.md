...
- [ ] **IG-28-003 P0** — Create keyboard and gamepad navigation for menus. *(Partial: **keyboard** navigation is done. `MenuModel` (`include/IronGang/UI/MenuModel.hpp`) holds items, selection, and activation with no drawing or input in it, so the parts that are easy to get subtly wrong are testable without a window: moving **skips disabled entries** rather than stepping onto them, wraps at both ends, counts enabled entries rather than raw indices for a multi-step move, and a menu with nothing enabled keeps its selection instead of spinning forever looking for one. Activating a disabled entry returns `MenuAction::None`, which is what stops it doing its thing anyway. Up/Down and W/S move, Enter/Space activate. **Gamepad navigation is not done** -- no gamepad input path exists anywhere in the game yet. Covered by `TestMenuModelSkipsDisabledAndWraps`.)*
- [ ] **IG-28-004 P0** — Create pause, settings, save/load, restart, and quit menus. *(Partial: **pause, save, load, restart, and quit** exist as a real navigable menu (IG-28-003) drawn on the pause screen; Escape toggles it, and quitting is an entry on it rather than a keystroke that ends the game from anywhere. The menu is rebuilt on every pause because what is available changes: **Load is disabled with "no save yet" until something has been written**, and **Restart from checkpoint is disabled with "no checkpoint reached"** until the mission records one -- shown greyed with the reason rather than hidden, since hiding an entry moves every entry below it under the player's fingers between two frames. Pausing is a safe save moment, so Save is always enabled there. Since 2026-08-26 the menu also carries **settings**: master volume (five steps, wrapping) and a HUD on/off toggle, each written to `runtime/settings.json` the moment it changes (plan_29 IG-29-005). They sit on the pause menu itself rather than behind a submenu -- with two settings, a submenu is a screen to learn for no benefit. Turning the HUD off never hides the pause menu, since a hidden menu is how someone gets stuck in a paused game. Still missing: a restart-from-the-beginning entry distinct from the checkpoint one, and any confirmation before quitting.)*
- [ ] **IG-28-005 P1** — Create UI layout scaling for aspect ratio, resolution, and DPI.
- [ ] **IG-28-006 P1** — Create safe-area support.
- [ ] **IG-28-007 P1** — Create basic rebinding (rebind the primary on-foot/vehicle/menu actions) for keyboard and gamepad; a full every-action matrix with cross-device conflict detection is deferred (see IG-28-024). *(Partial: **keyboard** rebinding exists as a model plus persistence, and **every rebindable key the game reads now goes through it** (`InputBindings`, `include/IronGang/Input/InputBindings.hpp`) -- sixteen actions in four groups, one table defining what exists, where it belongs, and its shipped key. Conflict detection is **per group**, which is what makes it useful rather than obstructive: Space is the handbrake while driving and confirms in a menu, and those never conflict because the game is never listening for both at once (see plan_28 IG-28-008's contexts); Global actions conflict with everything. `Rebind` displaces the loser, **reports which action lost the key** so a UI can say so, and costs the loser only the key it lost (a secondary is promoted rather than discarded). Bindings live in `runtime/settings.json`, written out in full so the file itself is the rebinding interface until a screen exists; an unknown action, an unknown key name, or a malformed entry keeps that action's default and warns, per action rather than costing the file. Driving and menu navigation **reuse** the movement bindings rather than owning a second set the player never chose. Covered by `TestInputBindingsDetectConflictsWithinContexts` and the binding half of `TestUserSettingsRoundTripAndFallBack`. **Missing: a rebinding screen** (the model reports everything one needs, but nothing draws it) and **gamepad**, which has no input path anywhere in the game.)*
