...
- [ ] **IG-27-002 P0** — Play one spatial ambient emitter and one vehicle sound through CNA. *(Still partial, but for a narrower reason than before. The vehicle sound is now genuinely **spatial** -- attenuated and panned from the active-camera listener through the `Vehicle` preset (`IG-27-003`/`IG-27-007`), as are the horn and footsteps. What is missing is the *ambient emitter*: there is no ambience asset in `assets/audio/` to place, and adding one is sourcing and licensing work (`assets/licenses/asset-registry.csv`), not engine work.)*
- [x] **IG-27-003 P0** — Create listener updates from active camera/player mode. *(`AudioListener` (`include/`/`src/Audio/AudioListener.hpp/.cpp`) is the **active camera** -- the same thing XNA's AudioListener means, and the right answer for a third-person game: the player hears what the shot shows. `IronGangGame::ComputeCameraPose()` was extracted so `Draw()`'s view matrix and `Update()`'s listener come from one function and cannot disagree about where the player is watching and listening from; on foot, driving and cutscene modes are therefore covered by construction rather than by three parallel branches. The camera-obstruction pull-in (plan_16 `IG-16-003`) moves the listener with it, deliberately: the listener is attached to the camera, not to a second idea of where the camera should be.)*
...
- [x] **IG-27-007 P1** — Create distance attenuation and spatialization presets. *(`ComputeSpatialGain()` returns the attenuation and pan CNA's `SoundEffect::Play(volume, pitch, pan)` actually takes -- not an HRTF and not doppler. Full volume inside a reference distance, linear rolloff to silence at a maximum (deliberately not inverse-square: at the volumes a game mixes at, a physically correct curve is either deafening up close or inaudible three metres away). Distance is 3D; **pan is XZ only**, because a source overhead has no left or right and treating height as lateral swings sounds across the stereo field whenever the camera pitches. Four presets -- `Voice` 6/35 m, `Vehicle` 12/90 m, `Effect` 8/45 m, `Ambience` 20/140 m -- so sounds of one kind share one distance instead of each call site inventing its own. The engine loop, horn and footsteps all play through it.)*
