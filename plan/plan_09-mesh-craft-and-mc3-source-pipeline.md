...
- [x] **IG-09-005 P0** — Author one production-path street prop set in MC3. *(`assets/source/mc3/street_lamp.mc3.xml`: base, post, arm and an emissive glass head, two materials (`iron` metallic 0.65, `lamp_glass` with `<emissive_color>`), 48 triangles. Generated through the real pipeline for the first time -- `cnjVersion 2`, four mesh parts on their own bones, `PbrEffect`, each carrying its own `diffuseColor`/`metallicFactor`/`roughnessFactor`/`emissiveFactor`. `PrototypeRenderer` substitutes it wherever the district authored a `lamp_west`/`lamp_east` box -- the same substitution the warehouse already uses, so the **box stays what the district means by "a lamp stands here"** and the model is only how it looks; placement, collision and the map are untouched. The old `lamp_glow_*` boxes are dropped because the head has its own emissive material, and east-side lamps are mirrored so the arm reaches the road from both pavements. Verified by colour: the head renders at exactly (203,175,104) = its authored glass colour times the sun, replacing the procedural glow. Budgeted (48/192 triangles, 2/8 materials) and registered.)*
- [ ] **IG-09-006 P1** — Define Iron Gang naming, units, axes, pivots, tags, and layer conventions for MC3.
- [ ] **IG-09-007 P1** — Define allowed MC3 collision values and required metadata.
- [ ] **IG-09-008 P1** — Define definition/instance conventions for repeated objects (lamps, benches, signs) authored once and placed many times.
