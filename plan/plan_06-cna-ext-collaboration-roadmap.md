...
- [x] **IG-06-021 P2** — Profile the first playable district against the 2-4GB RAM / 512MB-1GB VRAM target from `docs/performance-targets.md` and record results. *(Two independent 900-draw Release EasyGL AMD `mixed` captures exercise the complete first district plus one real transition. They record 177.1/177.6 MiB RAM and identical 55.574 MiB complete per-process DRM residency, far inside the 2 GiB/512 MiB minimum guards; exact frame/CPU/GPU/load results and archive hashes are retained in `docs/performance-baseline.md`. The runs are offscreen and therefore close this resource-profiling task, not physical-presentation gate M12.)*
