...
- [ ] **IS-39-007 P0** — Gate M6: one skinned character plays blended locomotion, dialogue pose, and vehicle entry/exit animations using cna-extended's Transform3/skinned-playback. *(Partial -- the first proving slice is done, blending/dialogue-pose/vehicle-entry are not: cna-extended gained `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT` (a new ECS component/system pair wrapping cna's general-purpose `Model`+`AnimationPlayer` skinned path, since the existing `SkinnedModelComponentEXT`/`AnimationSystem3DEXT` only wrap the separate Avatar-specific `SkinnedModelEXT` data model cna's own glTF/CNJ importer does not populate). A hand-authored (MC3 has no rigging support) 3-bone test character (`assets/source/gltf/test_character.gltf` -> `assets/generated/models/cnj/test_character.cnj`) plays "Idle"/"Walk" clips selected from live on-foot input in `IronShadowsGame::Update`, replacing the procedural player box via `PrototypeRenderer`. Verified: cna-extended's own component/system tests (clip logic + a real pixel-level render test) and a `--smoke` run of the full game with no crash. Explicitly NOT done: clip blending (hard cuts only -- not in scope of either system yet), a dialogue-pose clip (dialogue leaves the character in whatever locomotion clip it was already playing), and vehicle entry/exit animation (the character is simply not drawn while driving, matching the box it replaced) -- and no visual/interactive verification of any of this, since this environment has no display. See `plan_18-characters-skeletons-and-animation.md` for the itemized breakdown.)*
