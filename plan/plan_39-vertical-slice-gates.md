...
- [ ] **IS-39-007 P0** — Gate M6: one skinned character plays blended locomotion, dialogue pose, and vehicle entry/exit animations using cna-extended's Transform3/skinned-playback. *(Partial -- locomotion (including blending) and a dialogue pose are done, vehicle entry/exit is not: cna-extended gained `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT` (a new ECS component/system pair wrapping cna's general-purpose `Model`+`AnimationPlayer` skinned path, since the existing `SkinnedModelComponentEXT`/`AnimationSystem3DEXT` only wrap the separate Avatar-specific `SkinnedModelEXT` data model cna's own glTF/CNJ importer does not populate), later extended with a per-bone `Matrix::Lerp` crossfade (`BlendDurationEXT`/`BlendFromSkinTransformsEXT`/`BlendedSkinTransformsEXT`) between clips. A hand-authored (MC3 has no rigging support) 3-bone test character (`assets/source/gltf/test_character.gltf` -> `assets/generated/models/cnj/test_character.cnj`) plays "Idle"/"Walk" clips selected from live on-foot input in `IronShadowsGame::Update`, crossfading between them, and replacing the procedural player box via `PrototypeRenderer`; a third "Dialogue" clip (a static "parade rest" leg stance, rotated about a different axis than Walk's swing so it reads as a genuinely distinct pose) plays while `DialogueSystem::IsActive()` is true. Verified: cna-extended's own component/system tests (clip logic, blend-math correctness against hand-derived numbers, and a real pixel-level render test), a standalone diagnostic confirming the Dialogue pose's leg-rotation math against hand-derived pivot-rotation values, and a `--smoke` run of the full game with no crash (smoke mode never dismisses the opening dialogue, so this run exercises the Dialogue clip continuously for its whole duration). Explicitly NOT done: vehicle entry/exit animation (the character is simply not drawn while driving, matching the box it replaced) -- and no visual/interactive verification of any of this, since this environment has no display. See `plan_18-characters-skeletons-and-animation.md` for the itemized breakdown.)*
