...
- [x] **IG-20-003 P0** — Connect pedestrian locomotion to animation states (walk/idle/turn). *(All three states exist and are chosen by `SelectPedestrianAnimation()` (`include/IronGang/Gameplay/PedestrianAnimation.hpp`) -- a pure function of `IsWalking()` / `IsTurningInPlace()` / `IsFleeing()`, testable without a graphics device. **Turn** is new on both sides: `Pedestrian` now rotates toward the heading its path asks for at `kPedestrianTurnRate` (3.5 rad/s) and stands still while the error exceeds `kPedestrianTurnInPlaceThreshold`, instead of snapping 180 degrees in one frame at the end of a two-point pavement; and a `Turn` clip (short, fast alternating steps -- a shuffle, not a stride) was added to `assets/source/gltf/gen_test_character_gltf.py`. `Reset()` now also faces a pedestrian along its path, which was invisible while turning was instantaneous but would otherwise make every pedestrian pivot the moment it spawned. `assets/generated` is not committed, so a checkout whose asset build predates the Turn clip falls back to Walk rather than freezing mid-pose. Fleeing deliberately still snaps: panic is not a considered pivot. Not done: no left/right turn distinction -- `GetTurnRate()` is exposed signed for when the model has separate clips.)*
- [ ] **IG-20-004 P0** — Make pedestrians avoid the player and moving vehicles at a basic level. *(Partial, and unchanged by IG-20-010's pedestrian-to-pedestrian work: still only flees the player's vehicle within a fixed radius; does not avoid other traffic or other pedestrians — see status note above.)*
...
- [x] **IG-20-010 P1** — Create sidewalk congestion avoidance so pedestrians do not stack or clip through each other. *(Two mechanisms, because stacking and clipping are different problems. **Lanes**: `Pedestrian::SetLaneOffset` shifts a pedestrian sideways off the path centreline, always to the right of its own heading, so the two directions of travel occupy two lanes and pass each other instead of meeting head-on -- the offset moves the position everything outside sees (rendering, witness checks, congestion queries), so it is not merely cosmetic. **Yielding**: `Pedestrian::Update` now takes the distance to the nearest pedestrian ahead in its own lane and slows from 2.0 m to a stop at 0.7 m, the same shape as `TrafficVehicle`'s following distance at walking scale, so a queue forms instead of a pile. A **fleeing** pedestrian ignores clearance on purpose: someone running from a car does not queue. The lane test itself moved to a shared `DistanceAheadInLane` (`Gameplay/LaneClearance.hpp`) that traffic and pedestrians both use, differing only in lane half-width (2.0 m vs 0.55 m) -- previously it was a private helper inside `IronGangGame.cpp` with the traffic width baked in. Covered by `TestPedestriansDoNotWalkThroughEachOther` and `TestLaneClearanceSeesOnlyWhatIsAhead`. **Cost:** the scan is O(n squared) -- 144 checks per ambient update at 12 pedestrians -- and is deliberately not a profiler counter yet, since adding one revises the report schema and its comparator contract; measured `ai_cpu` maximum went from 0.022 ms to 0.094 ms. A streamed population (IG-20-008) needs both a counter and a spatial index.)*
- [ ] **IG-20-011 P1** — Create blocked-path recovery when a pedestrian's route is obstructed.
- [x] **IG-20-012 P1** — Create pedestrian crossing behavior at marked crosswalks, respecting traffic signals. *(`PedestrianMayCross()` / `PedestrianCrossingClearance()` (`include/`/`src/Gameplay/PedestrianCrossing.hpp/.cpp`) plus `SidewalkGraph::BuildCrossingPaths()`: two pedestrians per crossing shuttle kerb to kerb, and are **held at the kerb as an obstacle** while the traffic they would step in front of still has green -- the same mechanism the pedestrian queue already uses (`IG-20-010`) and the same one traffic vehicles use for a red light (plan_21 `IG-21-007`). One idea, used three times, rather than a third state machine. Amber does **not** permit crossing even though vehicles must stop for it: a car already committed to the junction is still coming through. Someone already out in the road past the kerb hold radius is never held, so a signal changing mid-crossing cannot freeze them in a live lane. An unsignalled crossing never holds anyone -- giving way is the driver's job, and waiting there would leave people at an empty road forever. A fleeing pedestrian ignores the signal. Not done: pedestrians do not route *onto* a crossing from a pavement -- the crossing shuttles are their own population, so nobody yet walks up the pavement, crosses, and continues on the other side.)*
