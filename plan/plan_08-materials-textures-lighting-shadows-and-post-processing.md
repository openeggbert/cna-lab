...
- [ ] **IG-08-014 P0** — Create a baked-lighting workflow for buildings (static lightmaps or vertex-baked ambient) as the primary lighting source. *(Partial. The **procedural** city already gets real per-face baked lighting via `LightmapMesh`/`DrawStaticCityMesh()`. **Imported** buildings now get the shared sun brightness applied to their own MC3 base colour (`ModelMaterialTable`, `assets/models/model-materials.json`, plus `PrototypeRenderer::ShadedBaseColor()`), which removed the deliberate "leave the warehouse at full brightness" exception -- on an untextured material that meant a pure-white slab brighter than anything else in the scene. **Correction (2026-08-26, later the same day): that diagnosis was of a stale asset, not the pipeline.** Once `mc3togltf` was built and the pipeline run end to end, a freshly generated `warehouse.cnj` came out as `cnjVersion 2` carrying `"diffuseColor": [0.42, 0.36, 0.31]`, `metallicFactor`, `roughnessFactor`, `emissiveFactor`, `alpha` and `"effect": "PbrEffect"` -- exactly its MC3 material. The `.cnj` files inspected were `cnjVersion 1`, generated months earlier by an older tool. What was actually true of that old asset: it dropped material data entirely -- a generated `.cnj` holds only vertices, indices, a vertex stride and an effect name, and the vertex layout (stride 32 = position/normal/uv) has **no colour channel**. Every MC3-sourced model therefore drew white; the sedan, authored dark red, drew pale grey. What remains open is the *baked* half: per-face ambient for imported geometry needs a colour channel to bake into, so it needs either a pipeline change or a rebuilt vertex buffer. Today an imported building gets one uniform brightness, not per-face shading.)*
