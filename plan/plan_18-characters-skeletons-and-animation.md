...
- [ ] **IS-18-006 P1** — Create animation assets for enter car, exit car, sit, drive, and steer. *(Partial: "EnterVehicle"/"ExitVehicle" one-shot clips exist (both legs bending together, as if sitting into/standing up from a seat) and drive a small `VehicleTransitionState` state machine in `IronShadowsGame` that keeps the character visible for 0.5s around the `playerDriving_` show/hide cut instead of an instant pop. No sit/drive/steer poses -- the character still is not drawn at all once actually driving.)*
