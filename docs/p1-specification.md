...
| Medicine | Verified in part | A skull indicates sickness. Marutchi traces required one or two doses depending on the saved illness state and used the same seven-phase, 16-frame action; the exact dose condition and treatment art for every other form remain open. |
...
| Child | Marutchi | Complete 28-phase awake home path, separate two-phase sleeping body and Z overlay, and seven-phase Medicine action transcribed |
...
selected P1 reference; Marutchi's complete awake path, separate sleeping body,
and the two-phase stacked-waste home overlay are also transcribed. Remaining
...
| Marutchi awake | A cleaned, daytime, complete-LCD 30 fps trace at 1× repeats a 28-phase path. The 10 × 9 long pose has single open eyes and one mouth row; the 10 × 8 short pose changes the eye spacing and uses two mouth rows. The origin sequence is `9, 7, 5, 7, 9, 11, 13, 11, 9, 10, 11, 12, 13, 11, 9, 7, 5, 7, 9, 11, 13, 11, 9, 10, 11, 12, 13, 11`, always at `y=3`. Matching stable phases repeat about every sixteen host frames (0.53 seconds); one- to three-frame incremental LCD writes are excluded. | All 28 origins, both exact open-eye row sets, pose order, cadence, direct wrap, and dedicated catalogue tests are implemented. | Capture the first teen home form next. |
| Marutchi Medicine | Complete 30 fps 1× recovery traces: after the skull clears, a front pose with the small dose mark alternates with two distinct side poses in the order front/A/front/B/front/A/front. Stable phases occupy `2, 2, 3, 2, 2, 2, 3` host frames, sixteen total. One saved illness cleared after one dose; another required two. | All three full 32 × 16 states, seven-phase order, nonuniform frame boundaries, completion, input blocking, and healthy-home return are implemented and tested. | Determine the dose condition and capture Medicine for every other form separately; do not reuse Marutchi art. |
| Marutchi sleep state | Complete-LCD 30 fps 1× trace: a separate closed-eye 10 × 9 / 10 × 8 body stays at `(11, 3)` and alternates every 27–28 host frames (0.92 seconds), independently from two Z arrangements every 24–25 host frames (0.82 seconds). The small arrangement occupies 7 × 6 cells at `(24, 0)` and the large Z occupies 4 × 6 cells at `(25, 2)`. Existing waste continues on its own cadence. | Awake and sleeping body catalogues are now separate. Both closed-eye body phases and exact Z phases, geometry, cadence, wrapping, independent timing, and waste composition are tested. | Capture sleep poses for every other form separately; do not infer that their body animation matches Marutchi. |
...
- Legacy tamagotchi-cna Pipple/Budbit forms, PEEK/NUM lines, random seed
