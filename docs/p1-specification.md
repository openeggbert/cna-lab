...
| Medicine | Verified in part | A skull indicates sickness. The observed Marutchi treatment uses one dose and a seven-phase, 16-frame action; dose counts and treatment art for every other form remain open. |
...
| Child | Marutchi | Two home silhouettes, sleep overlay, and seven-phase Medicine action transcribed; clean-state path provisional |
...
| Marutchi Medicine | One complete 30 fps 1× recovery trace: after the skull clears, a front pose with the small dose mark alternates with two distinct side poses in the order front/A/front/B/front/A/front. Stable phases occupy `2, 2, 3, 2, 2, 2, 3` host frames, sixteen total. One dose cured this observed illness. | All three full 32 × 16 states, seven-phase order, nonuniform frame boundaries, completion, input blocking, and healthy-home return are implemented and tested. | Capture Medicine for every other form separately; do not reuse Marutchi art. |
