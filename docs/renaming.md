...
`Iron Shadows` is a provisional working title. Rename it before public branding when a proper clearance process selects the final name.

## Identifiers that must change together

| Current value | Purpose | Example replacement form |
|---|---|---|
| `Iron Shadows` | Human-readable title | `Final Title` |
| `IronShadows` | CMake project, namespace, class prefixes | `FinalTitle` |
| `iron_shadows` | CMake targets, executable, save filename | `final_title` |
| `iron-shadows` | Install path and save format slug | `final-title` |
| `IRON_SHADOWS` | CMake options and compile definitions | `FINAL_TITLE` |
| `IS-` | Plan task prefix | Optional stable project prefix |

## Rename procedure

1. Commit or copy the repository so the operation can be reviewed.
2. Rename `include/IronShadows/` and `IronShadowsGame.hpp/.cpp`.
...
```bash
grep -RInE 'Iron Shadows|IronShadows|iron_shadows|iron-shadows|IRON_SHADOWS' .
```
