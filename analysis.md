# Iron Gang Technical and Product Analysis

## Document status

- Project name: **Iron Gang** (provisional and intentionally easy to rename)
...
- Engine dependency stack: sharp-runtime → CNA → cna-extended → Iron Gang (see section 5)
...
Iron Gang is technically feasible as an original historical open-city action-adventure using the supplied CNA, sharp-runtime, and Mesh Craft repositories. The available code already covers a meaningful portion of the low-level platform:
...
The provisional title **Iron Gang** was selected because it suggests an industrial historical atmosphere without copying another product's identity. The setting name **Iron City** is similarly provisional. Both are centralized in documentation and data so they can be changed later.
...
CNA's own examples demonstrate direct 3D rendering with vertex and index buffers, `BasicEffect`, matrices, depth testing, keyboard input, and indexed primitives. The Iron Gang scaffold deliberately follows these real interfaces.

CNA's example/shader-test tree goes further than section 6.3 of an earlier revision of this document assumed: it contains working shadow-mapping examples (`easygl_shadowmapping_createshadowmap_shader_test.cpp`, `..._drawwithshadowmap_shader_test.cpp`), GPU instancing tests across EasyGL, D3D9, Vulkan, and WebGPU, and a post-processing effect example. Treat these as the starting point for Iron Gang rendering, not as a reason to design a parallel "CNA EXT" material/shadow/instancing system from scratch.
...
Iron Gang currently uses `System::IO::File` and `System::IO::Directory` in its save path, proving that sharp-runtime is integrated as a real dependency rather than merely named in CMake.

sharp-runtime should remain infrastructure, not become the game engine itself. It is appropriate for data, I/O, tasks, and utility layers. Rendering, physics, entity simulation, mission semantics, and navigation should remain explicit Iron Gang/CNA systems.
...
The included Iron Gang MC3 prototype was validated against the supplied `mc3.xsd`. It represents a road crossing, several buildings, a warehouse, a mission marker, lights, PBR material data, static collision metadata, and trigger metadata.

Mesh Craft itself (the application, distinct from the MC3 data format) is a real C++23 3D scene editor, not just a schema/CLI toolchain: a Dear ImGui application with a live 3D viewport, orbit camera, transform gizmos, a scene hierarchy panel and properties panel, CSG boolean operations, extrude-along-path authoring, an animation timeline, autosave/recovery, glTF/GLB import as bounded editable MC3 content, and a first-person **Walk Mode** that exercises walk-collision proxies directly in the editor. It also has a bounded **Preview/Play** mode that dispatches timer, Walk Mode area-enter/exit, and picked-object click event bindings through an isolated, resource-limited Lua VM for authoring feedback — this is editor-side authoring convenience, not the shipped game's runtime scripting (Iron Gang's own mission scripting, per group 24, is deliberately simpler: engine-evaluated condition/action expressions, not a general embedded VM). Practically, this means Iron Gang's spatial content — buildings, roads, interiors, props, collision, and mission trigger/marker areas — is authored by placing and manipulating objects directly in Mesh Craft's viewport, not by hand-editing MC3 XML text.

Mesh Craft should be treated as an authoring system for hand-built content. Iron Gang content (buildings, roads, interiors) is manually authored in MC3/Blender rather than produced by procedural generators; Mesh Craft's preview/action runner is useful for authoring feedback but should not silently become the complete production game runtime, and building a generator toolchain on top of it is explicitly out of scope for now (see section 7.3). Production mission, physics, streaming, and AI semantics should be owned by the game layer.

### 3.4 cna-extended observations

`cna-extended` is a sibling repository (`../cna-extended`), wired into Iron Gang's `CMakeLists.txt` and linked into `iron_gang_core`. It is a C++23 port of MonoGame.Extended (2D collision/quadtree, tweening, screens, sprite/tilemap rendering, 2D particles, an Artemis-style ECS) plus an additive, less battle-tested `World3DEXT` layer: a `Transform3` scene-graph hierarchy, 3D collision shapes with octree broadphase, `ModelComponentEXT`/`SkinnedModelComponentEXT` rendering components, and a single-clip skinned-animation driver (`AnimationSystem3DEXT`) that advances playback and recomputes bone transforms with no blending or state machine.

It is **not** a material/PBR/shadow/instancing/post-processing system — none of that exists in cna-extended, and it does not need to, because CNA already provides it (section 3.1). Its own `CMakeLists.txt` detects an existing `CNA` target from a parent project and links against it rather than building its own copy, which is exactly how Iron Gang consumes it. Use it for the ECS, scene hierarchy, 3D collision/octree, and skinned-model playback boilerplate a small team should not rewrite from scratch; still plan to add real animation blending/transitions/IK as scoped Iron Gang work, since neither CNA nor cna-extended provides it today.
...
cna-extended (ECS, Transform3 hierarchy, 3D collision/octree, skinned-model playback)
      ↓
Iron Gang reusable game systems
      ↓
Iron Gang content and campaign
```
...
The integration should hide the selected library behind Iron Gang interfaces:
...
CNA already contains the foundations for skinned models and animation clips, and cna-extended's `ModelComponentEXT`/`SkinnedModelComponentEXT` plus `AnimationSystem3DEXT` already provide single-clip skinned-model rendering and playback (advancing playback and recomputing bone transforms) for cna's Avatar-specific `SkinnedModelEXT` data model. Gate M6 (below) found and closed a real gap alongside that: cna's *other* skinned-playback path — a real `Model` whose `Tag` holds a `SkinningData`, animated via `AnimationPlayer`, which is what cna's own glTF/CNJ import tools actually populate for a skinned asset — had no ECS component/system pair at all until `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT` were added to cna-extended for this milestone, which also gained a basic per-bone `Matrix::Lerp` crossfade between clips (simple linear interpolation, not true rotation-aware blending — adequate for a short crossfade between similar poses, not a full blend-space/layered system). What is still missing — in both CNA and cna-extended — and remains real, scoped Iron Gang work is richer blending and transition logic on top of that playback:

- true rotation-aware clip blending (today's crossfade lerps raw matrices, a documented simplification) and richer transition rules;
...
CNA already offers low-level and media audio capabilities. Iron Gang needs a game-level audio graph:
...
- all Iron Gang `.cpp` translation units passed a C++23 syntax-only compile using the real CNA and sharp-runtime headers and software-backend definitions;
...
Mitigation: cna-extended's 2D port is well-trodden (MonoGame.Extended heritage); its `World3DEXT` addition is newer and less proven. Exercise the ECS/Transform3/collision/skinned-playback paths Iron Gang actually needs early and with real content, rather than assuming they are as solid as the 2D core.
...
The locked target for Iron Gang is the full Mafia 1-equivalent campaign scope: multiple districts plus a countryside area, roughly 15-20 missions, connected by loading screens (section 8). This vertical slice is not a permanently tiny demo; it is the first of several district-sized repetitions that add up to that campaign, each one reusing the same proven systems rather than inventing new ones per district.
...
6. Select and prototype a physics library behind an abstraction (done: Jolt Physics v5.6.0, MIT, pinned in `THIRD_PARTY.md`; `IronGang::Physics::PhysicsWorld` in `include/`/`src/Physics/` hides Jolt's own types behind a PIMPL boundary; character, trigger, raycast, and 4-wheel vehicle prototypes are all proven in `tests/PhysicsTests.cpp`, gate M4. Also done: `PlayerController`'s on-foot movement and `VehicleController`'s driving are now physics-driven -- a `JPH::CharacterVirtual` capsule and a `JPH::VehicleConstraint`/`WheeledVehicleController` respectively, world geometry collision comes from real static bodies built from `PrototypeWorld`'s colliders plus a dedicated ground plane. Vehicle engine/transmission/suspension tuning still uses Jolt's own generic defaults, not a deliberately-tuned "feel" -- unverified visually since this repository has no display/interactive test access; see `NEXT.md`).
7. Create a second, genuinely different district to validate the district-loading approach (section 8), not a bigger single streamed sector (done, gate M5: `DistrictManager` in `include/`/`src/World/` owns the currently loaded `PrototypeWorld` and its static physics bodies; a second `Countryside` district was added alongside the original `WarehouseBlock`, each with its own exit trigger back to the other. A transition destroys the old district's static bodies, constructs the new `PrototypeWorld`, rebuilds its bodies and renderer geometry (`PrototypeRenderer::RebuildStaticGeometry`), and shows a loading screen with a 0.6s minimum display time; mission state and the player's driving/vehicle state carry across, and `SaveGame` now persists the current district id. Verified by `TestDistrictTransition` in `tests/CoreTests.cpp` (round-trip static-physics-body-count leak checks, one-shot arrival signaling) and a `--smoke` run with no crash. Still open: background/async loading, a longer soak test, and per-district mutable world state beyond spawn/exit points -- see `plan/plan_13-world-partitioning-and-streaming.md` and `plan/plan_39-vertical-slice-gates.md` gate M5 for the itemized breakdown).
...
10. Add a small in-engine cinematic sequence that hands control back to the mission (done, gate M8, camera-track-only scope): new `CutscenePlayer`/`CutsceneSequence` (`include/`/`src/Cutscenes/`) play a hand-written, versioned JSON camera keyframe sequence (`assets/cutscenes/prologue_intro.cutscene.json`) -- a 2.5s pan from an establishing shot of the warehouse delivery target to the exact framing of the normal gameplay follow-camera at the player's spawn point, authored so the handoff back to gameplay has no visible pop. Started alongside the opening dialogue in `IronGangGame::Initialize()`; pressing Enter once dialogue has finished but the cutscene is still playing skips straight to its terminal camera state (`CutscenePlayer::Skip()`, identical to a natural finish); `LoadPrototype()`/`ResetPrototype()` force it to that same terminal state defensively for save-safety; player/vehicle input and physics stay frozen while it plays, the same mechanism dialogue already uses. Verified: 3 new unit tests (advance/interpolate/finish, skip-applies-terminal-state, and 5 malformed-data validation cases + a missing file all correctly rejected), a standalone diagnostic confirming the real committed file parses correctly, and `--smoke` runs covering both mid-playback and full natural completion with no crash. Not done: animation/dialogue/audio/event/fade tracks (camera-track only) or a timeline debug overlay -- see `plan/plan_26-cutscenes-and-cinematic-sequencing.md` and `plan/plan_39-vertical-slice-gates.md` gate M8 for the itemized breakdown.
11. Prove one skinned character end to end with blended locomotion, a dialogue pose, and vehicle entry/exit animation, all at prototype fidelity (gate M6, now fully done): cna-extended gained `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT`, a new ECS component/system pair wrapping cna's general-purpose `Model`+`AnimationPlayer` skinned path (the one its own glTF/CNJ import tools actually populate), since the existing `SkinnedModelComponentEXT`/`AnimationSystem3DEXT` only wrap the separate Avatar-specific `SkinnedModelEXT` data model; it later gained a per-bone `Matrix::Lerp` crossfade (`BlendDurationEXT`/`BlendedSkinTransformsEXT`) between clips. A hand-authored 3-bone test character (Mesh Craft/MC3 has no rigging/skinning authoring support, so this deliberately bypasses it -- see `assets/source/gltf/test_character.gltf`'s own provenance note) plays "Idle"/"Walk" clips selected from live on-foot input, crossfading between them; a "Dialogue" clip (legs angled together about a different axis than Walk's swing) while `DialogueSystem::IsActive()`; and "EnterVehicle"/"ExitVehicle" one-shot clips (legs bending together) driving a small `VehicleTransitionState` state machine in `IronGangGame` that keeps the character briefly visible around the `playerDriving_` show/hide cut. Verified: cna-extended's own component/system tests (clip-switching logic, blend-math correctness against hand-derived numbers, plus a real pixel-level render test), standalone diagnostics confirming each new pose's leg-rotation math, and a `--smoke` run of the full game with no crash (smoke mode never dismisses the opening dialogue, so it exercises the Dialogue clip continuously, but never presses 'E', so the vehicle-transition state machine itself was verified by code review, not an automated test). Not done or verified: sit/drive/steer poses while actually driving (character still isn't drawn at all once driving), and any interactive/visual verification (no display in this environment) -- see `plan/plan_18-characters-skeletons-and-animation.md` and `plan/plan_39-vertical-slice-gates.md` gate M6 for the itemized breakdown.
12. Make the one existing mission genuinely data-driven (gate M7, done at prototype fidelity): `assets/missions/prologue.mission.json` (a pre-existing stub file from the original scaffold, previously just documentation of an "intended future form" -- now the real, active definition) defines the prologue delivery mission's 5 states, each with an objective-text string, a named transition condition (`dialogue_finished`/`player_near_vehicle`/`player_driving`/`player_driving_in_warehouse_goal`), and a next-state id, loaded via a new `MissionDefinition`/`LoadMissionDefinition` (`include/`/`src/Missions/`, parsed with sharp-runtime's own `System::Text::Json`, already a linked dependency -- no new library needed) with inline validation (duplicate ids, dangling references, unknown condition names, empty state lists all rejected with an actionable error). `PrototypeMission` now looks up state definitions instead of a hardcoded switch, while keeping its enum-based public API and `SaveGame` compatibility unchanged -- deliberately not a general condition/action expression language (`plan/plan_24-mission-framework-and-scripting.md`'s own explicit non-goal), matching the locked "hand-written data files, no bespoke scripting" scope decision. Falls back to an identical hardcoded default (matching `DialogueSystem::LoadFallbackPrologue()`'s convention) if the file fails to load/validate. Verified: the existing `TestMissionFlow` (hardcoded default, unchanged) plus two new tests -- `TestMissionLoadsCommittedFile` (the real committed file drives the identical flow) and `TestMissionValidationRejectsMalformedData` (6 malformed/missing-file cases correctly rejected, one well-formed mission still loads afterward) -- and a `--smoke` run with no crash. See `plan/plan_24-mission-framework-and-scripting.md` and `plan/plan_39-vertical-slice-gates.md` gate M7 for the itemized breakdown of what's done vs. still open (typed mission variables, richer condition/action sets, failure/retry policies, campaign graph, etc.).
13. Populate the first district with Mafia-1-fidelity traffic, pedestrians, and one police-response scenario (gate M9, done at first-pass/prototype fidelity, including the gate's own literal "ten-minute soak" wording): a new shared `WaypointPath`/`AdvanceAlongPath()` (`include/`/`src/World/WaypointPath.hpp/.cpp`) -- a hand-authored ordered polyline, not a graph -- underlies both new `TrafficVehicle` (accelerates toward a cruise speed, brakes smoothly for an obstacle ahead within a fixed gap) and new `Pedestrian` (walks a fixed sidewalk path, overridden by a timed flee-directly-away state near the player's vehicle), `include/`/`src/Gameplay/`. New `PoliceSystem` runs a `Clear -> Dispatched -> Chasing` state machine off a simplified fixed-radius "witness" check (not real vision-cone/line-of-sight): a witnessed speeding/close-proximity offense dispatches a patrol car after a fixed delay, which then drives straight at the player (ignoring roads); a second patrol car joins after a fixed escalation time (the one locked escalation tier); the chase resolves back to Clear once sustained beyond a fixed distance. `PrototypeWorld::BuildWarehouseBlock()` hand-authors one traffic loop and two sidewalk paths (`BuildCountryside()` has none, by design); `IronGangGame` spawns/ticks/renders all three systems (2 traffic vehicles, 2 pedestrians) every frame regardless of dialogue/cutscenes, gated only on district transitions, with none of this ambient state persisted to `SaveGame`. Verified: 4 new deterministic unit tests including a full `Clear -> Dispatched -> Chasing -> escalate -> resolve` cycle against hand-computed, standalone-diagnostic-confirmed values (one diagnostic caught a wrong test assumption about wrap-on-arrival timing before it shipped), the full `ctest` suite, `./scripts/check-syntax.sh`, two short `--smoke` runs, and the gate's own "ten-minute soak" requirement itself: a `--smoke 3000` run, launched in the background and timed via `date +%s` before/after, ran for 980 real seconds (~16.3 simulated minutes -- CNA's fixed-timestep `Game::Tick()` accumulator tracks real elapsed time 1:1, confirmed by reading `cna/src/Microsoft/Xna/Framework/Game.cpp`, so there is no way to compress this into a shorter run) with all three systems ticking every frame, exiting cleanly with no crash, error, or fallback-load message. Not done: traffic signals, real vision-cone perception, lane graphs/route-following for patrol cars, local avoidance, 10-20 pedestrians/3-5 vehicles (only 2 each), and any debug overlay -- see `plan/plan_19-navigation-and-pathfinding.md`, `plan/plan_20-pedestrians-and-ambient-ai.md`, `plan/plan_21-traffic-simulation.md`, `plan/plan_22-police-witnesses-crime-and-wanted-response.md`, and `plan/plan_39-vertical-slice-gates.md` gate M9 for the itemized breakdown.

14. Bring the first district's vertical slice to production-path assets, baked lighting, one dynamic sun, limited shadows, audio, and a real UI (gate M10, all five pieces done at first-pass/prototype fidelity): a hand-built bitmap `SpriteFont` (public-domain font8x8 glyphs, since CNA has no XNB font pipeline) drives a real on-screen HUD, replacing the window-title-only display. A single shared sun direction (`SunLight.hpp`) is applied as a CPU-computed per-actor `DiffuseColor` brightness multiplier to every dynamic actor (confirmed `BasicEffect`'s built-in lighting is a no-op on the SOFTWARE backend, but `DiffuseColor` itself is not), and as a real baked lightmap texture atlas for the static procedural city mesh (one flat-shaded tile per box face, sampled via `DualTextureEffect`, confirmed fully implemented on that backend) -- MC3-sourced models stay out of scope, no lightmap UV channel in that pipeline yet. "Limited shadows" are simple alpha-blended ground-decal blob shadows under the player and their own vehicle (real shadow-mapping confirmed not achievable without modifying CNA). Audio is real CC0 sound (Nox Sound Design's "Essentials Series" pack, itch.io) rather than synthesized placeholders -- the user downloaded the 988MB pack manually after the automated free-download flow proved impossible to complete headlessly (a JS-gated payment-bypass step, no connected browser in this environment); a looped engine idle, a horn, and a footstep one-shot were extracted and wired up via `SoundEffect`/`SoundEffectInstance`, with no ambience/siren content available in the chosen pack. Verified: new unit tests for the sun-brightness formula and the lightmap tile-baking math (both cross-checked against hand/Python calculations), the full `ctest` suite, `./scripts/check-syntax.sh`, `--smoke` runs, and a standalone diagnostic that exercised real audio playback end to end. Measured a real ~4-5x per-frame rendering slowdown from the lightmap's two-texture-sample draw path, not yet profiled against performance budgets (gate M12 scope) -- see `plan/plan_39-vertical-slice-gates.md` gate M10, and `plan/plan_08`, `plan/plan_27`, `plan/plan_28` for the itemized per-area breakdown.

15. Prove the first district's complete vertical-slice mission end to end -- happy-path, failure, retry, save/load, cutscene-skip, district-transition, a soak, a performance capture, and a license audit (gate M11, all ten sub-tasks done): unlike gate M10, this needed almost no new production code, just tests proving what already exists. Five new integration tests in `tests/CoreTests.cpp` cover save/load mid-mission (proving the loaded mission can still complete, not just that the state enum round-trips), cutscene-skip not blocking mission progression, `Reset()` acting as a "retry" back to the mission's initial state, a save made on foot far from a parked vehicle restoring both positions independently ("vehicle-loss recovery," reinterpreted since no combat/damage system exists yet -- `plan_23`), and a full district round trip mid-mission leaving mission state untouched. Fresh-start playthrough and missing-optional-asset behavior were already covered by pre-existing tests. A `--smoke 3000` soak ran 65 minutes (deliberately stopped, not crashed -- the M10 lightmap draw path made it far slower per frame than the M9 baseline) with no error logged, more than six times the gate's own "ten minutes" wording. A performance capture measured ~55MB peak memory (far under the ~2-4GB budget) and ~1.55s/frame on this environment's CPU software rasterizer -- not meaningfully comparable to the 30-60 FPS target, since that backend was never intended to be performant; real frame-time verification needs a GPU-backed build not yet available here. A license audit found and fixed two real gaps: two original data files missing from the asset registry, and a now-false claim in `THIRD_PARTY.md` that no external content was bundled.
...
The guiding philosophy for Iron Gang is: **full Mafia 1 (2002) content scope, at Mafia 1-era system fidelity — not a second engine, not modern-open-world-scale simulation.** Concretely, that means the full campaign target (multiple districts plus countryside, roughly 15-20 missions, section 23) built on discrete district loading rather than seamless streaming (section 8), traffic/pedestrian/police AI tuned to what Mafia 1 itself actually simulated rather than GTA-scale systems (sections 9.4, 14.2, 14.3), materials/shadows/instancing/post-processing integrated from CNA rather than redesigned (section 6.3), and content authored by hand in Mesh Craft/MC3 rather than generated by bespoke tooling (section 7.3, section 17). Over-simplifying past that point would make the game "Mafia lite," which is explicitly not the goal; over-building past it would repeat the mistake of designing a second engine or a modern open-world simulation nobody asked for.

The decisive engineering task is to integrate what already exists (CNA, cna-extended, Mesh Craft) into a disciplined game layer, not to endlessly expand low-level APIs before a mission exists. Iron Gang should grow through vertical slices: each slice must integrate content creation, conversion, rendering, collision, gameplay, audio, mission state, and persistence, at the fidelity level locked in this document. The current repository is the first such scaffold and is designed to be replaced incrementally rather than discarded.
