...
11. Prove one skinned character end to end (gate M6), not full blended-locomotion/dialogue-pose/vehicle-entry animation (partial, gate M6): cna-extended gained `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT`, a new ECS component/system pair wrapping cna's general-purpose `Model`+`AnimationPlayer` skinned path (the one its own glTF/CNJ import tools actually populate), since the existing `SkinnedModelComponentEXT`/`AnimationSystem3DEXT` only wrap the separate Avatar-specific `SkinnedModelEXT` data model. A hand-authored 3-bone test character (Mesh Craft/MC3 has no rigging/skinning authoring support, so this deliberately bypasses it -- see `assets/source/gltf/test_character.gltf`'s own provenance note) plays "Idle"/"Walk" clips selected from live on-foot input, replacing the procedural player box in `PrototypeRenderer`. Verified: cna-extended's own component/system tests (clip-switching logic plus a real pixel-level render test) and a `--smoke` run of the full game with no crash. Explicitly not done: clip blending (hard cuts only), a dialogue-pose clip, vehicle entry/exit animation (character simply isn't drawn while driving), and any interactive/visual verification (no display in this environment) -- see `plan/plan_18-characters-skeletons-and-animation.md` and `plan/plan_39-vertical-slice-gates.md` gate M6 for the itemized breakdown.
