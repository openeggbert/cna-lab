...
CNA already contains the foundations for skinned models and animation clips, and cna-extended's `ModelComponentEXT`/`SkinnedModelComponentEXT` plus `AnimationSystem3DEXT` already provide single-clip skinned-model rendering and playback (advancing playback and recomputing bone transforms) for cna's Avatar-specific `SkinnedModelEXT` data model. Gate M6 (below) found and closed a real gap alongside that: cna's *other* skinned-playback path — a real `Model` whose `Tag` holds a `SkinningData`, animated via `AnimationPlayer`, which is what cna's own glTF/CNJ import tools actually populate for a skinned asset — had no ECS component/system pair at all until `ModelAnimationComponentEXT`/`ModelAnimationSystem3DEXT` were added to cna-extended for this milestone, which also gained a basic per-bone `Matrix::Lerp` crossfade between clips (simple linear interpolation, not true rotation-aware blending — adequate for a short crossfade between similar poses, not a full blend-space/layered system). What is still missing — in both CNA and cna-extended — and remains real, scoped Iron Shadows work is richer blending and transition logic on top of that playback:
