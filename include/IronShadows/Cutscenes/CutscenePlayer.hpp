...
{
    // Gate M8: the minimal in-engine sequence player (IS-26-001) -- plays a CutsceneSequence's
    // camera track, supports skipping straight to the terminal (last-keyframe) state
    // (IS-26-004), and reports when it has finished so the caller can hand control back to
    // gameplay. Deliberately holds no player-control/freeze logic itself (IronShadowsGame decides
    ...
    public:
        ...
        // finish (IS-26-004: skip must apply the same required terminal state as a full play-through).
        void Skip();
